...
schedule_i_location = {}
schedule_i_location.__index = schedule_i_location
...
function schedule_i_location.new(name)
    local self = setmetatable({}, schedule_i_location)
    if name then
...
function schedule_i_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        ...
        if existing then
            ...
            exit = schedule_i_location.new(exit)
        end
    end
    if rule == nil then
        rule = always
...
function schedule_i_location:connect_two_ways(exit, rule)
...
function schedule_i_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function schedule_i_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function schedule_i_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function schedule_i_location:connect_two_ways_stuck(exit, rule1, rule2)
...
function schedule_i_location:accessibility()
...
-- 
function schedule_i_location:discover(accessibility, keys)
...
entry_point = schedule_i_location.new("entry_point")
